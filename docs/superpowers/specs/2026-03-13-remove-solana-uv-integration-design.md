...
- `src/vaultsign/crypto/solana.py` — SolanaSigner class (Ed25519 signing via solders)
...
| File | Changes |
|------|---------|
| `src/vaultsign/cli.py` | Remove `"solana"` from `_TYPE_MAP` and `click.Choice`. Delete `_derive_from_mnemonic` Solana branch (bip_utils import + BIP44 derivation). Remove `"solana"/"ed25519"` branch from `_derive_address`. Fix `list_keys` display logic that falls back to `"solana"` for unknown key types. |
| `src/vaultsign/server.py` | Remove top-level `from .crypto.solana import SolanaSigner`. Remove `self._solana` attribute, its loading in unlock, its zeroization in lock, and the `"solana"` case in `_get_chain_signer`. |
| `src/vaultsign/client.py` | Remove `self.solana = _ChainClient(self._send, "solana")`. Update `_ChainClient` docstring (currently says "evm or solana"). Keep `_ChainClient` abstraction and `self.evm`. |
| `src/vaultsign/keystore.py` | Remove `"ed25519"` branch in `_derive_address_from_key` and its `SolanaSigner` import. Remove `"ed25519": "solana"` from `chain_types` dict in `add_key`. |
...
| Update README | Replace `pip install vaultsign` with `uv sync`. Show `uv run vaultsign ...` and `uv run pytest`. |
